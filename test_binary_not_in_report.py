...
def grid_search_cv(model_type='tree', scoring='recall'):
...
def random_search_cv(model_type='tree',scoring='recall'):
...
def objective(trial, model_type='tree',scoring='recall'):
...
def plot_feature_importances(model, model_name='Model'):
...
def evaluate_models(model_type='tree', model_name='Decision Tree'):
    ...
    basic_cv_scores = cross_val_score(base_model, x_train, y_train, cv=skf, scoring='recall')
    # Get mean score
    basic_cv_mean = basic_cv_scores.mean()
    print(f"Base Model CV Scores: {basic_cv_scores}")
    print(f"Base Model Mean CV Recall: {basic_cv_mean:.4f}")
    print(f"Base Model Std CV Recall: {basic_cv_scores.std():.4f}")
    ...
    print(f"Bayesian Optimization Best CV Recall: {best_score_bayes:.4f}")
    ...
    plot_feature_importances(final_model, model_name)
    # Get predictions
    y_pred = final_model.predict(x_test)
    
    # Evaluate model
    test_f1, test_recall = evaluate_model_metrics( y_test, y_pred)
    ...
    return test_f1, best_params, best_method_name,test_recall



def main():
    # Evaluate Decision Tree
    dt_test_f1, dt_params, dt_method, dt_test_recall = evaluate_models('tree', 'Decision Tree')
    
    # Evaluate Random Forest
    rf_test_f1, rf_params, rf_method, rf_test_recall = evaluate_models('random forest', 'Random Forest')
    ...
    scores = [dt_test_recall, rf_test_recall]
    colors = ["blue", "red"]
